...
describe('battleSim computeBattleOutcome', () => {
  test('strong hero defeats single orc (deterministic hits)', () => {
    ...
    const hero = { id: 'h1', name: 'Strong', hp: 30, atk: 12, mp: 0, currentTask: 'IDLE' as any, classId: 'WARRIOR' as any };
  ...
  test('weak hero likely fails vs two orcs (deterministic misses/hits)', () => {
    ...
    const hero = { id: 'h2', name: 'Weak', hp: 5, atk: 1, mp: 0, currentTask: 'IDLE' as any, classId: 'WARRIOR' as any };
    ...
    expect(outcome.casualties.find((c) => c.heroId === 'h2')?.hpAfter ?? 0).toBeLessThanOrEqual(hero.hp);
  });
});
